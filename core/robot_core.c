...
/*    if (fabs(back_diff_v_ref) > max_lin_acc && fabs(*linear_speed)>fabs(last_v_ref))
        *linear_speed = last_v_ref + (max_lin_acc * Hz_4)*(*linear_speed / fabs(*linear_speed));
    if (fabs(back_diff_w_ref) > max_ang_acc && fabs(*angular_speed)>fabs(last_w_ref))
        *angular_speed = last_w_ref + (max_ang_acc * Hz_4)*(*angular_speed / fabs(*angular_speed));
*/
    /************/

    /*From linear and angular speed to each engine velocity*/
//    printf("linear: %f  \t angular: %f\n",*linear_speed, *angular_speed);
	get_vel_motori_constant_ratio(linear_speed, angular_speed, &v_m1, &v_m2);
...
//    last_v_ref = *linear_speed;
//    last_w_ref = *angular_speed;
}
